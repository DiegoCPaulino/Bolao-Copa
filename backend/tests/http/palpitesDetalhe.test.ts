...
describe.skipIf(!temBanco)("rotas da Rodada-detalhe (8.4-A, HTTP autenticado)", () => {
  ...
  it("palpite SINGULAR: registra (200), lê de volta, e relançar ATUALIZA sem duplicar", async () => {
    ...
    const reg = await put(
      `/api/participantes/${anaId}/rodadas/${rodadaId}/jogos/${jogoId}/palpite`,
      {
        golsEsquerda: 2,
        golsDireita: 1,
      },
    ...
    const lido = await get(`/api/participantes/${anaId}/rodadas/${rodadaId}/palpites`);
    ...
    await put(`/api/participantes/${anaId}/rodadas/${rodadaId}/jogos/${jogoId}/palpite`, {
      golsEsquerda: 3,
      golsDireita: 1,
    });
    const lido2 = await get(`/api/participantes/${anaId}/rodadas/${rodadaId}/palpites`);
  ...
  it("sob-demanda: lançar resultado → pontuação/resumo/tabela já vêm RECALCULADOS", async () => {
    ...
    const res = await put(`/api/jogos/${jogoId}/resultado`, { golsEsquerda: 2, golsDireita: 1 }); // cravou
    expect(res.statusCode).toBe(200);

    const pont = await get(`/api/rodadas/${rodadaId}/pontuacao`);
    ...
    const resumo = await get(`/api/jogos/${jogoId}/resumo`);
    ...
    const tabela = await get(`/api/rodadas/${rodadaId}/tabela`);
  ...
  it("erros: 404 rodada/participante (palpite), 404 jogo (resumo), 400 jogo de outra rodada", async () => {
    ...
    expect(
      (await put(`/api/participantes/${anaId}/rodadas/nao-existe/jogos/${jogoId}/palpite`, corpo))
        .statusCode,
    ).toBe(404);
    expect(
      (
        await put(
          `/api/participantes/nao-existe/rodadas/${rodadaId}/jogos/${jogoId}/palpite`,
          corpo,
        )
      ).statusCode,
    ).toBe(404);
    expect((await get("/api/rodadas/nao-existe/pontuacao")).statusCode).toBe(404);
    expect((await get("/api/rodadas/nao-existe/tabela")).statusCode).toBe(404);
    expect((await get("/api/jogos/nao-existe/resumo")).statusCode).toBe(404);

    // jogo que existe mas é de OUTRA rodada → JogoForaDaRodada → 400
    const outra = await setup();
    const fora = await put(
      `/api/participantes/${anaId}/rodadas/${rodadaId}/jogos/${outra.jogoId}/palpite`,
      corpo,
    );
  ...
  it("400 palpite inválido (gols negativos) e 400 resumo sem resultado", async () => {
    ...
    const semResultado = await get(`/api/jogos/${jogoId}/resumo`);
  ...
  it("sem cookie → 401 (rotas novas nascem protegidas)", async () => {
    expect((await app.inject({ method: "GET", url: "/api/rodadas/x/pontuacao" })).statusCode).toBe(
      401,
    );
    expect((await app.inject({ method: "GET", url: "/api/jogos/x/resumo" })).statusCode).toBe(401);
    expect(
      (
        await app.inject({
          method: "PUT",
          url: "/api/participantes/p/rodadas/r/jogos/j/palpite",
  ...
  it("coexistência: o palpite PLURAL continua funcionando", async () => {
    const { rodadaId, jogoId, anaId } = await setup();
    const plural = await put(`/api/participantes/${anaId}/rodadas/${rodadaId}/palpites`, {
