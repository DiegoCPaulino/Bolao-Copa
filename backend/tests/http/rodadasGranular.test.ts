...
describe.skipIf(!temBanco)("rotas granulares de rodada/jogo (HTTP, autenticado)", () => {
  ...
  it("monta uma rodada INTEIRA incrementalmente: criar vazia → +jogos → editar → remover → estado → detalhar", async () => {
    ...
    const criada = await post("/api/rodadas", { fase: "OITAVAS" });
    ...
    const ap1 = await post(`/api/rodadas/${rodadaId}/jogos`, {
      selecaoEsquerdaId: a,
      selecaoDireitaId: b,
    });
    ...
    const ap2 = await post(`/api/rodadas/${rodadaId}/jogos`, {
      selecaoEsquerdaId: c,
      selecaoDireitaId: d,
    });
    ...
    const editado = await put(`/api/jogos/${j1Id}`, { selecaoEsquerdaId: a, selecaoDireitaId: c });
    ...
    const removido = await del(`/api/jogos/${j2Id}`);
    ...
    const detalhe = await get(`/api/rodadas/${rodadaId}`);
  ...
  it("DELETE de jogo COM palpites → 409 (JogoComPalpites); o jogo continua lá", async () => {
    ...
    const jogoId = (
      await post(`/api/rodadas/${rodadaId}/jogos`, { selecaoEsquerdaId: a, selecaoDireitaId: b })
    ).json().jogos[0].id;
    ...
    const res = await del(`/api/jogos/${jogoId}`);
    expect(res.statusCode).toBe(409);
    expect(res.json()).toMatchObject({ erro: { codigo: "JOGO_COM_PALPITES" } });
    // não removeu
    expect((await get(`/api/rodadas/${rodadaId}`)).json().jogos).toHaveLength(1);
  });

  it("erros: rodada/jogo inexistente → 404; par igual → 400", async () => {
    expect(
      (
        await post("/api/rodadas/nao-existe/jogos", {
          selecaoEsquerdaId: "x",
          selecaoDireitaId: "y",
        })
      ).statusCode,
    ...
    expect(
      (await put("/api/jogos/nao-existe", { selecaoEsquerdaId: "x", selecaoDireitaId: "y" }))
        .statusCode,
    ).toBe(404);
    expect((await del("/api/jogos/nao-existe")).statusCode).toBe(404);

    // par igual (mesma seleção dos dois lados) → JogoInvalido → 400
    const criada = await post("/api/rodadas", { fase: "OITAVAS" });
    const a = await novaSelecao();
    const parIgual = await post(`/api/rodadas/${criada.json().id}/jogos`, {
      selecaoEsquerdaId: a,
      selecaoDireitaId: a,
    });
  ...
  it("coexistência: POST atômico {fase, jogos[≥1]} → 201; jogos:[] → 400; sem cookie → 401", async () => {
    const [a, b] = await Promise.all([novaSelecao(), novaSelecao()]);
    const atomico = await post("/api/rodadas", {
      fase: "FINAL",
    ...
    expect((await app.inject({ method: "POST", url: "/api/rodadas" })).statusCode).toBe(401);
  });
});
